...
void setup() {
  ...
  //strcpy(NSSID, wm.getWiFiSSID().c_str());
  //strcpy(NPASS, wm.getWiFiPass().c_str());
  //conn = new WiFiConnectionHandler(NEWSSID, NEWPASS);
...
void loop() {
  ...
  Serial.println(map(throttleval,300,4000,0,255));
  handleSpeedLimit();
}
...
void handleMotordriver(){
  ...
  ledcWrite(speedChannel, map(throttleval,300,4000,0,255));//pwm signal for speed
}
